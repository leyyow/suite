  ...
  <div ref="menuRef" class="relative inline-block text-left">
    ...
    >
      <div
        ...
        ref="dropdownRef"
        :class="[
          'absolute z-[9999] w-max min-w-40 py-2 px-2 bg-white shadow-lg rounded-md divide-y divide-brand-100',
      ...
      >
        ...
        <div v-for="item in items" :key="item.label" class="py-0.5 px-px">
          <button
            :class="[
              'group flex w-full items-center gap-2 px-2 py-2 text-sm font-medium',
              'text-brand-400 hover:text-brand-500 hover:bg-brand-100 rounded-md',
              item.class,
            ]"
            @click.stop="
              () => {
